...
if __name__ == "__main__":
    ...
    model_module = importlib.import_module('.%s' % 'DST', 'lib.models.DST')
    model = model_module.DST().to(cfg.DEVICE)
    ...
    model.temporal_modeling.smpl = SMPL(
        SMPL_MODEL_DIR,
        batch_size=64,
        create_transl=False,
        gender=gender
    ...
    with torch.no_grad():
        ...
        for seq_name in pbar:
            ...
            for curr_idx in range(0, len(chunk_idxes), 8):
                ...
                preds, mask_ids, smpl_output_global = model(input_feat, J_regressor=J_regressor, is_train=False)
