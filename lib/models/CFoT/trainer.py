...
class Trainer():
    ...
    def train(self):
        ...
        for i in range(self.num_iters_per_epoch):
            ...
            preds, pred_global = self.generator(inp, is_train=True)
            
            timer['forward'] = time.time() - start
            start = time.time()

            gen_loss, loss_dict = self.criterion(
                generator_outputs_global=pred_global,
            ...
            kp_2d_loss.update(loss_dict['loss_kp_2d_global'].item(), inp.size(0))
            kp_3d_loss.update(loss_dict['loss_kp_3d_global'].item(), inp.size(0))
            ...
            accel_loss_global_2d.update(loss_dict['loss_accel_2d_global'].item(), inp.size(0))
            accel_loss_global_3d.update(loss_dict['loss_accel_3d_global'].item(), inp.size(0))
            ...
            summary_string = f'({i + 1}/{self.num_iters_per_epoch}) | Total: {bar.elapsed_td} | ' \
                             ...
                             f'| 3d_short: {kp_3d_loss_short.avg:.2f} 2d_global_accel: {accel_loss_global_2d.avg:.2f} ' \
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, target in enumerate(self.valid_loader):
                ...
                inp = target['features']
                batch = len(inp)
                preds, pred_global = self.generator(inp, is_train=False, J_regressor=J_regressor)
