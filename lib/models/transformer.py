...
"""
"""
class Cam_Transformer(nn.Module):
    def __init__(self, depth=3, embed_dim=128, mlp_hidden_dim=256, h=8, 
        ...
        self.init_pose = torch.from_numpy(mean_params['pose'][:]).unsqueeze(0).to('cuda')                       # 24 * 6
        self.init_shape = torch.from_numpy(mean_params['shape'][:].astype('float32')).unsqueeze(0).to('cuda')   # 10
        self.init_cam = torch.from_numpy(mean_params['cam']).unsqueeze(0).to('cuda')                            # 3
